...
description=[[
Checks if an open socks proxy or open http proxy is running on the target.
a Request Granted response from the target port.

The payloads try to open a connection to myip.ipip.net port 80.  A
...
local function custom_test(host, port, test_url, pattern)
  ...
  -- make requests
  status4, get_r4, cstatus4,result_td = proxy.test_get(host, port, "socks4", test_url, hostname, pattern)
  status5, get_r5, cstatus5,result_td = proxy.test_get(host, port, "socks5", test_url, hostname, pattern)

  local get_status,result_td = proxy.test_get(host, port, "http", test_url, hostname, pattern)
  local head_status,result_td = proxy.test_head(host, port, "http", test_url, hostname, pattern)
  local conn_status,result_td = proxy.test_connect(host, port, "http", hostname)
  ...
  if not (cstatus4 or cstatus5) then
    return false, nil,lstatus, response
  else
...
local function default_test(host, port)
  ...
  local test_url = "http://myip.ipip.net"
  local hostname = "myip.ipip.net"
  local pattern  = "当前 IP："
  status4, get_r4, cstatus4,result_td = proxy.test_get(host, port, "socks4", test_url, hostname, pattern)
  status5, get_r5, cstatus5,result_td = proxy.test_get(host, port, "socks5", test_url, hostname, pattern)
  get_status, get_r1, get_cstatus,result_td = proxy.test_get(host, port, "http", test_url, hostname, pattern)
  local _
  head_status, _, head_cstatus,result_td = proxy.test_head(host, port, "http", test_url, hostname, pattern)
  conn_status,result_td = proxy.test_connect(host, port, "http", hostname)

  fstatus = status4 or status5
  if(cstatus4) then response[#response+1] = "socks4 " .. result_td end
  if(cstatus5) then response[#response+1] = "socks5 " .. result_td end
  if(fstatus) then return fstatus, response end
  if get_status then fstatus = true; response[#response+1] = "get " .. result_td end
  if head_status then fstatus = true; response[#response+1] = "head " .. result_td end
  if conn_status then cstatus = true; response[#response+1] = "connent " .. result_td end
  ...
  portrule = shortport.port_or_service({1080, 9050,8123,3128,8000,8080},
  {"socks", "socks4", "socks5", "tor-socks",'polipo','squid-http','http-proxy'})
